...
{
  GRLUA_DEBUG_CALL;
  std::cout << "nh_sphere " << counter << std::endl;
  counter++;
...
int gr_material_cmd(lua_State* L)
{
  ...
  double reflectance = luaL_checknumber(L, 4);
  double transmittance = luaL_checknumber(L, 5);

  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     glm::vec3(ks[0], ks[1], ks[2]),
                                     shininess, reflectance, transmittance);
...
int gr_node_set_texture_map_cmd(lua_State* L)
{
  ...
  self->m_textureMap = new Image(name);
  std::cout << "finish Loading textureMap" << std::endl;
...
int gr_node_set_bump_map_cmd(lua_State* L)
{
  ...
  self->m_bumpMap = new Image(name);
  std::cout << "finish Loading bumpMap" << std::endl;
  self->hasBump = true;
...
extern "C"
int gr_node_set_perlin_noise_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  unsigned int seed = luaL_checknumber(L, 2);
  self->m_perlinNoise = new PerlinNoise(seed);
  std::cout << "finish Loading PerlinNoise" << std::endl;
  self->hasPerlinNoise = true;

  return 0;
}

extern "C"
int gr_node_set_movement_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  glm::vec3 iv;
  get_tuple(L, 2, &iv[0], 3);
  self->m_initialVelocity = iv;

  glm::vec3 acceleration;
  get_tuple(L, 3, &acceleration[0], 3);
  self->m_acceleration = acceleration;

  std::cout << "finish loading movement" << std::endl;
  self->isMoving = true;
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"set_perlin_noise", gr_node_set_perlin_noise_cmd},
  {"set_movement", gr_node_set_movement_cmd},
